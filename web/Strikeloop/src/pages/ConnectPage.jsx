...
import { runAnalysis, runDiagramAnalysis, validateAwsCredentials } from '../lib/api.js'
...
export default function ConnectPage() {
  ...
  async function handleConnectAWS() {
    ...
    try {
      ...
      if (result.valid) {
        ...
        setAwsError(result.error || 'Invalid credentials. Please check your Access Key ID and Secret Access Key.')
      }
    } catch (err) {
      setAwsError(err.message || 'Could not reach the backend. Is the server running?')
    } finally {
      setAwsLoading(false)
    }
  }
  ...
  async function handleRunAnalysis() {
    ...
    try {
      ...
      if (accessKey.trim() && secretKey.trim()) {
        payload.aws_access_key_id = accessKey.trim()
        payload.aws_secret_access_key = secretKey.trim()
      }
  ...
  function handleDiagramFile(file) {
    ...
    if (!allowed.includes(file.type)) {
      setDiagramError('Please upload a PNG, JPG, GIF or WebP image.')
      return
    }
  ...
  async function handleDiagramAnalysis() {
    if (!diagramFile || diagramLoading) return
    if (!awsConnected) {
      setDiagramError('Connect your AWS account first — needed for AI analysis via Bedrock.')
      return
    }
    setDiagramLoading(true)
    setDiagramError(null)
    try {
      const data = await runDiagramAnalysis({
        file: diagramFile,
        aws_access_key_id: accessKey.trim() || undefined,
        aws_secret_access_key: secretKey.trim() || undefined,
      })
      setJobId(data.job_id)
      addRunRecord({
        jobId: data.job_id,
        repo: `Diagram: ${diagramFile.name}`,
        createdAt: new Date().toISOString(),
        status: 'running',
      })
      navigate(`/run/${data.job_id}`)
    } catch (err) {
      setDiagramError(err.message || 'Failed to start diagram analysis.')
      ...
      <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 12 }}>
              ...
              {awsAccountId && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, paddingLeft: 22 }}>
                  <span style={{ fontSize: 11, color: '#52525B' }}>Account:</span>
                  <span style={{ fontFamily: 'Geist Mono, monospace', fontSize: 11, color: '#22C55E', letterSpacing: '0.05em' }}>
                    {awsAccountId}
                  </span>
                </div>
              )}
        ...
        <div style={{ marginTop: 12 }}>
          ...
          <button
            ...
            style={{
              width: '100%',
              background: diagramOpen ? '#141414' : 'transparent',
              border: `1px solid ${diagramOpen ? '#2A2A2A' : '#1A1A1A'}`,
              borderRadius: 13,
              padding: '15px 22px',
              display: 'flex',
              alignItems: 'center',
            ...
            onMouseEnter={e => { if (!diagramOpen) e.currentTarget.style.borderColor = '#2A2A2A' }}
            onMouseLeave={e => { if (!diagramOpen) e.currentTarget.style.borderColor = '#1A1A1A' }}
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ fontSize: 22 }}>📐</span>
              <div style={{ textAlign: 'left' }}>
                <p style={{ fontSize: 14, fontWeight: 500, color: '#F5F5F5', margin: 0 }}>Analyse from Architecture Diagram</p>
                <p style={{ fontSize: 12, color: '#52525B', margin: '2px 0 0' }}>Upload a PNG/JPG diagram — no GitHub repo needed</p>
              </div>
            </div>
            <span style={{ fontSize: 13, color: '#52525B', transition: 'transform 200ms', transform: diagramOpen ? 'rotate(180deg)' : 'rotate(0deg)' }}>▼</span>
          </button>
          ...
          {diagramOpen && (
            <div style={{
              ...
              {/* Drop zone */}
              <div
                onDragOver={e => { e.preventDefault(); setDiagramDragging(true) }}
                onDragLeave={() => setDiagramDragging(false)}
                onDrop={e => {
                  e.preventDefault()
                  setDiagramDragging(false)
                  handleDiagramFile(e.dataTransfer.files[0])
                }}
                onClick={() => document.getElementById('diagram-file-input').click()}
                style={{
                  border: `2px dashed ${diagramDragging ? '#4A4A4A' : diagramFile ? '#22C55E40' : '#2A2A2A'}`,
                  borderRadius: 10,
                  padding: diagramPreview ? 0 : '32px 20px',
                  textAlign: 'center',
                  cursor: 'pointer',
                  transition: 'border-color 200ms',
                  overflow: 'hidden',
                  background: diagramDragging ? '#1A1A1A' : 'transparent',
                  minHeight: diagramPreview ? 0 : 120,
                  display: 'flex',
                  flexDirection: 'column',
                  alignItems: 'center',
                  justifyContent: 'center',
              ...
              >
                {diagramPreview ? (
                  ...
                  <>
                    ...
                    <p style={{ fontSize: 13, color: '#A1A1AA', margin: 0 }}>Drag & drop your architecture diagram here</p>
                    <p style={{ fontSize: 11, color: '#3F3F3F', marginTop: 4 }}>PNG, JPG, GIF, WebP supported</p>
                  </>
                )}
              </div>
              ...
              {diagramFile && (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 10 }}>
                  <span style={{ fontSize: 12, color: '#A1A1AA', fontFamily: 'Geist Mono, monospace' }}>
                    {diagramFile.name} · {(diagramFile.size / 1024).toFixed(0)} KB
                  </span>
                  <button
                    onClick={e => { e.stopPropagation(); setDiagramFile(null); setDiagramPreview(null); setDiagramError(null) }}
              ...
              {/* Analyse button */}
              <button
                onClick={handleDiagramAnalysis}
                disabled={!diagramFile || diagramLoading}
                style={{
                  marginTop: 14,
                  width: '100%',
                  background: diagramFile ? '#F5F5F5' : '#1F1F1F',
                  color: diagramFile ? '#0A0A0A' : '#52525B',
                  border: 'none',
                  borderRadius: 10,
                  padding: '12px 18px',
                  fontSize: 14,
                  fontWeight: 500,
                  cursor: diagramFile && !diagramLoading ? 'pointer' : 'not-allowed',
                  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                  transition: 'all 200ms',
                }}
              >
                {diagramLoading ? <><Spinner />Analysing diagram…</> : '📐  Analyse diagram →'}
              </button>

              {!awsConnected && (
                <p style={{ fontSize: 11, color: '#52525B', textAlign: 'center', marginTop: 8 }}>
                  Connect AWS above first — required for AI analysis via Bedrock
                </p>
              )}
            </div>
          )}
