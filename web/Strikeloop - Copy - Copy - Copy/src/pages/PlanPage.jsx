        ...
        <>
          ...
          <div className="bg-card border border-border rounded-xl overflow-hidden mb-8 animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: '200ms', animationFillMode: 'both' }}>
            ...
            {plan.resources.map((res, i) => {
              const meta = ACTION_META[res.action] || ACTION_META.create
              const ActionIcon = meta.icon
              const isLast = i === plan.resources.length - 1
              return (
                <div
                  key={res.name}
                  className={`flex items-center gap-3 px-5 py-3 hover:bg-secondary/30 transition-colors duration-100 ${isLast ? '' : 'border-b border-border'}`}
                >
                  <ActionIcon className="w-4 h-4 shrink-0" style={{ color: meta.color }} />
